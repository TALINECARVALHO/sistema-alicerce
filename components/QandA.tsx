          ...
          demand.questions.map((q) => (
            <div key={q.id} className="p-4 border border-slate-200 rounded-lg bg-slate-50">
              <p className="text-sm font-semibold text-slate-800">
                <span className="font-bold text-blue-600">{q.supplierName}</span> perguntou em {new Date(q.askedAt).toLocaleString('pt-BR')}:
              </p>
              ...
              {q.answer ? (
                <div className="mt-4 pt-3 border-t border-slate-200">
                  <p className="text-sm font-semibold text-slate-800">
                    <span className="font-bold text-green-600">{q.answeredBy}</span> respondeu em {new Date(q.answeredAt!).toLocaleString('pt-BR')}:
                  </p>
